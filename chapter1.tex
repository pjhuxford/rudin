...
\begin{questions}
  ...
  \begin{solution}
    ...
    So we have demonstrated that for every complex number $w$, there exists a complex number $z$ for which $z^2=w$. It is clear that $z=0$ iff $w=0$, so provided $w=0$ we actually have two complex square roots, namely $z$ and $-z$. Moreover, let $x$ be a complex number with $x^2=w$. Then we have
