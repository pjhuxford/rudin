...
\begin{questions}
  ...
  \begin{solution}
    Suppose that $\vect{z}$ satisfies $\abs{\vect{z}-\vect{x}}=\abs{\vect{z}-\vect{y}}=r$. Then the Schwarz inequality gives
    \begin{align*}
      d^2 = \abs{\vect{x}-\vect{y}}^2 &= \abs{(\vect{x}-\vect{z}) + (\vect{z}-\vect{y})}^2 \\
                                      &= \abs{\vect{x}-\vect{z}}^2 + 2(\vect{x}-\vect{z})\cdot(\vect{z}-\vect{y}) + \abs{\vect{z}-\vect{y}}^2 \\
                                      &\leq \abs{\vect{x}-\vect{z}}^2 + 2\abs{\vect{x}-\vect{z}}\abs{\vect{z}-\vect{y}} + \abs{\vect{z}-\vect{y}}^2 \\
                                      &= (2r)^2.
    \end{align*}
    Hence if there are any $\vect{z}$ satisfying the equation, then $d\leq2r$, which answers (c).

    If $2r=d$ then equality must hold in the above application of the Schwarz inequality. By \textbf{15}., since $r>0$ we conclude there is a $\lambda\in\R$ with $\vect{x}-\vect{z}=\lambda(\vect{z}-\vect{y})$. Then
    \[ r = \abs{\lambda(\vect{z}-\vect{y})} = \abs{\vect{z}-\vect{y}} \implies \abs{\lambda}=1. \]
    If $\lambda=-1$, then $\vect{x}=\vect{y}$, contradicting $d>0$. Hence $\lambda=1$, and so $\vect{z}=(\vect{x}+\vect{y})/2$. So if $2r=d$, there is precisely one such $\vect{z}$, which answers (b).
  ...
  \begin{solution}
    \begin{align*}
      \abs{\vect{x}+\vect{y}}^2 + \abs{\vect{x}-\vect{y}}^2 &= (\vect{x}+\vect{y})\cdot(\vect{x}+\vect{y}) + (\vect{x}-\vect{y})\cdot(\vect{x}-\vect{y}) \\
                                                            &= (\vect{x}\cdot\vect{x} + 2\vect{x}\cdot\vect{y} + \vect{y}\cdot\vect{y}) + (\vect{x}\cdot\vect{x} - 2\vect{x}\cdot\vect{y} + \vect{y}\cdot\vect{y}) \\
                                                            &= 2\vect{x}\cdot\vect{x} + 2\vect{y}\cdot\vect{y} \\
                                                            &= 2\abs{\vect{x}}^2 + 2\abs{\vect{y}}^2.
    \end{align*}
  \end{solution}
  ...
  \begin{solution}
    Suppose that $\vect{x}=(x_1,x_2,\ldots,x_k)$. If any one of the $x_j$ is equal to 0, then we may define $\vect{y}=(y_1,y_2,\ldots,y_k)$, where $y_j=1$ and $y_i=0$ whenever $i\neq j$. As $k\geq2$, $\vect{y}\neq\vect{0}$, and
    \[ \vect{x}\cdot\vect{y} = \sum_{i=1}^k x_iy_i = x_jy_j + \sum_{i\neq j} x_iy_i = 0\cdot1 + \sum_{i\neq j} x_i\cdot0 = 0. \]
    Otherwise all of $x_1,x_2,\ldots,x_k$ are nonzero. Since $k\geq 2$, in particular $x_1$ and $x_2$ are nonzero. Hence if we define $\vect{y}=(y_1,y_2,\ldots,y_k)$ by $y_1=x_2$, $y_2=-x_1$ and $y_i=0$ if $i>2$, then $\vect{y}\neq\vect{0}$. Moreover
    \[ \vect{x}\cdot\vect{y} = \sum_{i=1}^k x_iy_i = x_1x_2-x_2x_1 + \sum_{i=3}^k x_i\cdot0 = 0. \]
    So if $k\geq2$, for every $\vect{x}\in\R^k$ there exists $\vect{y}\in\R^k$ such that $\vect{y}\neq\vect{0}$ but $\vect{x}\cdot\vect{y}=0$. However if $k=1$ then we can take $x=1$, then whenever $x\cdot y=0$ we must have $y=0$, therefore the same result is not also true in this case.
  \end{solution}
  ...
  \begin{solution}
    ...
    \begin{align*}
      4\abs{\vect{x}-\vect{b}}^2 - \abs{\vect{x}-\vect{a}}^2 &= 4\abs{\vect{x}}^2 - 8\vect{x}\cdot\vect{b} + 4\abs{\vect{b}}^2 - \abs{\vect{x}}^2 + 2\vect{x}\cdot\vect{a} - \abs{\vect{a}}^2 \\
                                                             &= 3\abs{\vect{x}}^2 - 2\vect{x}\cdot(4\vect{b} - \vect{a}) + 4\abs{\vect{b}}^2 - \abs{\vect{a}}^2 \\
                                                             &= 3\abs*{\vect{x}-\frac{1}{3}(4\vect{b}-\vect{a})}^2 - \frac{1}{3}\abs{4\vect{b}-\vect{a}}^2 + 4\abs{\vect{b}}^2 - \abs{\vect{a}}^2 \\
                                                             &= 3\abs*{\vect{x}-\frac{1}{3}(4\vect{b}-\vect{a})}^2 - \frac{16}{3}\abs{\vect{b}}^2 + \frac{8}{3}\vect{b}\cdot\vect{a} - \frac{1}{3}\abs{\vect{a}}^2 + 4\abs{\vect{b}}^2 - \abs{\vect{a}}^2 \\
                                                             &= 3\abs*{\vect{x}-\frac{1}{3}(4\vect{b}-\vect{a})}^2 - \frac{4}{3}\abs{\vect{b}}^2 + \frac{8}{3}\vect{b}\cdot\vect{a} - \frac{4}{3}\abs{\vect{a}}^2 \\
                                                             &= 3\abs*{\vect{x}-\frac{1}{3}(4\vect{b}-\vect{a})}^2 - \frac{4}{3}\abs{\vect{b}-\vect{a}}^2.
    \end{align*}
    Hence $\abs{\vect{x}-\vect{a}}=2\abs{\vect{x}-\vect{b}}$ if and only if $\abs{\vect{x}-\frac{1}{3}(4\vect{b}-\vect{a})}=\frac{2}{3}\abs{\vect{b}-\vect{a}}$. So if we set
    \[ \vect{c}=\frac{1}{3}(4\vect{b}-\vect{a}),\quad r=\frac{2}{3}\abs{\vect{b}-\vect{a}}, \]
    then $\abs{\vect{x}-\vect{a}}=2\abs{\vect{x}-\vect{b}}$ if and only if $\abs{\vect{x}-\vect{c}}=r$.
  \end{solution}
